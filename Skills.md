...
Gunakan easing universal yang elegan (misal: cubic-bezier(0.16, 1, 0.3, 1) atau ease-out-expo). Hindari animasi linear yang kaku.
...
Phase 2 (Map User Request): Analisis apa yang ingin diubah oleh user. Klasifikasikan apakah ini perubahan struktur (layout), penambahan konten, atau modifikasi alur.
...
### Mengapa struktur `skills.md` di atas sangat cocok untuk pendekatan Anda?

1. **Adanya "Execution Matrix" untuk Animasi (Section 2 - Skill 3):** Ini memaksa AI untuk tidak pernah lupa membuat mikro-interaksi. AI akan selalu mengecek: *"Apakah tombol ini sudah ada efek hovernya? Apakah card ini sudah ada transisinya saat kursor mendekat?"*
2. **Prinsip "Component Adaptability" (Section 2 - Skill 2):** Saat Anda meminta AI membuat komponen baru yang kebetulan tidak ada di template Figma, instruksi ini memastikan AI akan "menjiplak" gaya visual yang sudah ada (misalnya tingkat kebulatan sudut/border-radius, style bayangan, dll) agar komponen baru tersebut tetap terlihat menyatu dengan sisa halaman lainnya.
3. **Guardrails yang Ketat (Section 4):** Melarang keras adanya *unstyled components*, sehingga Anda akan selalu mendapatkan hasil akhir berkode matang yang sudah siap pakai dengan interaksi yang dinamis.

Silakan gunakan file spesifikasi di atas untuk AI Agent Anda agar ia memahami alur kerja redesain interaktif berbasis Figma ini secara maksimal! Beri tahu saya jika ada bagian aturan atau pustaka animasi tertentu (seperti Tailwind, Framer Motion, atau GSAP) yang ingin Anda kunci di dalam instruksinya.
